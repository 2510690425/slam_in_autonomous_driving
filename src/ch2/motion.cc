...
int main(int argc, char** argv) {
    ...
    /// 可视化
    auto ui = std::make_shared<sad::ui::PangolinWindow>();
    if (ui->Init() == false) {
        return -1;
    }
    std::weak_ptr<sad::ui::PangolinWindow> ui_ptr(ui);
    ...
    while (!ui_ptr.expired()
        ...
        usleep(dt * 1e6); // 休眠50ms
    }
    if(ui_ptr.expired()) {
        LOG(INFO) << "UI is shutdown by CTRL+C";
        ui.reset();
        return 1;
    }
    else
    {
        LOG(INFO) << "UI is shutdown by click the exit button";
        ui->Quit();
        ui.reset();
    }
    return 0;
